...
class LLM:
    def __init__(self, model: ollama.Client, database: Database):
        """
    ...
    def process_user_query(self, user_query: str) -> str:
        ...
        if status == "Table_Names":
            ...
            response = self.model.generate(MODEL_NAME, user_query).response
            return response
    ...
    def handle_database_query(self, user_query: str) -> str:
        ...
        if "columns" not in self.context:
            ...
            self.context["columns"] = self.match_columns(user_query, available_columns)

        print(self.context)

        # Step 3: Handle conditions
        if "conditions" not in self.context:
            # get the current data information based on the table name and coulmns
            query = f"SELECT {", ".join(self.context['columns'])} FROM {self.context["table"]}"
            print(f"Before condition query: {query}")
            data = self.execute_sql_query(query)
            self.context["conditions"] = self.extract_conditions(user_query, data)
    ...
    def match_table_name(self, user_query: str, available_tables: List) -> str:
        ...
        return (
            response.response.strip() if response.response.strip() != "None" else None
        )
    ...
    def match_columns(self, user_query: str, available_columns: List) -> List:
        ...
        response = self.model.generate(
            ...
            - If the user expects the data for all columns or if you are clear on about columns which user is refering, return all 'column1, column2', etc. 
            - If the user expects any specific column or columns return a comma-separated list of columns like 'column1' or 'column1, column2', etc.
            Do not include any extra explanation or text.""",
        )
        return response.response.strip().split(",")

    def extract_conditions(self, query: str, data: str) -> str:
        """
        ...
        response = self.model.generate(
            ...
            where the table name is {self.context['table']} and the columns are {self.context['columns']}. And below the available data from the databse 
            based on columns: \n{data}\n
            Based on above data, identify the WHERE conditions the user is referring to. 
